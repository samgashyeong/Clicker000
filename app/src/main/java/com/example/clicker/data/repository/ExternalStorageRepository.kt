...
class ExternalStorageRepository(private val context: Context) {
    ...
    fun findClickFile(content: String) {
...
//                ).path}")
        MediaScannerConnection.scanFile(context, arrayOf("${
            Environment.getExternalStoragePublicDirectory(
            DIRECTORY_DOWNLOADS
            ).path}/${CLICKER000_EXTERNAL_FILE_NAME}.json"), null) { path, uri ->
            ...
            saveClickFile(content)
        }
    }

    private fun saveClickFile(content: String) {
        val resolver = context.contentResolver
        val contentValues = ContentValues().apply {
            put(MediaStore.MediaColumns.DISPLAY_NAME, CLICKER000_EXTERNAL_FILE_NAME)
